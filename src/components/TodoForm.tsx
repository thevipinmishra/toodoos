...
import { Button, Input, ListBox, ListBoxItem, Popover, Select, SelectValue, TextField } from "react-aria-components";
...
export function TodoForm({ onSubmit }: TodoFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2">
      <TextField autoComplete="off" spellCheck="off" value={title} aria-label="Add new todo" onChange={setTitle} className="flex-1">
        <Input 
      ...
      >
        ...
        <Popover className="w-32">
          <ListBox className="rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
            <ListBoxItem id={Priority.LOW} className="cursor-pointer rounded-md px-3 py-2 text-sm text-gray-600 outline-none hover:bg-gray-100 focus:bg-gray-100">
              Low
            </ListBoxItem>
            <ListBoxItem id={Priority.MEDIUM} className="cursor-pointer rounded-md px-3 py-2 text-sm text-gray-600 outline-none hover:bg-gray-100 focus:bg-gray-100">
              Medium
            </ListBoxItem>
            <ListBoxItem id={Priority.HIGH} className="cursor-pointer rounded-md px-3 py-2 text-sm text-gray-600 outline-none hover:bg-gray-100 focus:bg-gray-100">
              High
            </ListBoxItem>
          </ListBox>
        </Popover>
